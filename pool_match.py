...
if __name__ == '__main__':
    ...
    if not os.path.exists(args.output_dir):
        os.makedirs(args.output_dir)
    if not os.path.exists(args.output_dir + '/outputs_match'):
        os.makedirs(args.output_dir + '/outputs_match')
    ...
    for epoch in range(args.epochs):
        ...
        save_image(test_img_syn, os.path.join(args.output_dir, 'outputs_match/img_{}.png'.format(epoch)))
        generator.train()
        ...
        if (epoch + 1) % args.eval_interval == 0:
            ...
            for e_idx, e_model in enumerate(args.eval_model):
                if top1s[e_idx] > best_top1s[e_idx]:
                    ...
                    torch.save(
                        model_dict,
                        os.path.join(args.output_dir, 'match_model_dict_{}.pth'.format(e_model)))
